...
const StoryImage: FC<StoryImageProps> = ( {
  ...
  const isSkeleton = !!( data.data?.source && !( data.data.source as { uri?: string } )?.uri && !data.isVideo );

  useEffect( () => {
    if ( isSkeleton && data.data ) {
      const animDur = data.data.animationDuration ?? 4000;
      loading.value = false;
      duration.value = animDur;
      if ( isActive.value ) {
        onLoad( animDur );
      }
    }
  }, [ isSkeleton, data.data?.id ] );
  ...
  return (
    <>
      ...
      <View style={[ ImageStyles.image, mediaContainerStyle ]}>
        {data.data?.source && ( data.data.source as { uri?: string } )?.uri ? (
          data.isVideo ? (
            <StoryVideo
          ...
          <View
            style={[ { width: WIDTH, aspectRatio: 0.5626 }, imageStyles, ImageStyles.imageSkeleton ]}
            onLayout={( e ) => onImageLayout( Math.min( HEIGHT, e.nativeEvent.layout.height ) )}
          />
        )}
      </View>
